...
const AddAuthorModal = ({ isOpen, onClose }) => {
  ...
  const fetchAuthors = async () => {
    try {
      const response = await axios.get("http://193.242.208.20:4000/api/authors");
  ...
  const handleAddAuthor = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("http://193.242.208.20:4000/api/authors", { name, bio });
  ...
  const handleDeleteAuthors = async () => {
    setLoading(true);
    try {
      await Promise.all(selectedAuthors.map(id => axios.delete(`http://193.242.208.20:4000/api/authors/${id}`)));
